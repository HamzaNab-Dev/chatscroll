        ...
        IEnumerable<string> existingFolders)
    {
        try
        {
            ...
            var prompt = $$"""
                ...
                - If a parent folder exists for the topic, suggest ONLY that parent folder path (e.g. existing "programming" → suggest "programming", NOT "programming.general" or "programming.solid_principles")
                - NEVER suggest a child folder named "general", "misc", "other", or any generic catch-all — if the parent fits, use it directly
                - Only suggest a new child folder if the subtopic is large and very distinct (e.g. "medicine.blood_thinners" when "medicine" exists and blood thinners are clearly a major subtopic)
                ...
                Naming rules — folder names must be plain everyday language:
                - Maximum 2 words per name
                - Use common words anyone would understand — no jargon, no acronyms
                - Use "Blood Thinners" not "Anticoagulants"
                - Use "Pain Relief" not "Analgesics"
                - Use "Design Patterns" not "SOLID Principles"
                - Use "Containers" not "Containerization Technology"
                - Use "Programming" not "General Programming"
                - Use "Medicine" not "Pharmacology"
                - Use "Heart Health" not "Cardiovascular Medicine"
                - Use "Data Storage" not "Database Architecture"
                """;
